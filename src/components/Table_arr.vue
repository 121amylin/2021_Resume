<template>
  <div class="section container">
    <div class="table_box">
      <div class="gg-arrow-align-h rwd_table_icon"></div>
      <div class="rwd_table">
        <table class="table">
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="(item, index) in get_work" :key="item.id">
              <th class="num">{{ index + 1 }}</th>
              <td class="pic">
                <img :src="require(`../assets${item.pic}`)" alt="" />
                <!-- {{require(`../assets${item.pic}`)}} -->
...
export default {
  ...
  computed: {
    get_work () {
      return this.$store.state.work.public_sector.work
    }
  }
}
</script>
